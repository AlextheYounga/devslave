...
const gitScriptsDir = path.join(paths.repoRoot, "src", "api", "scripts", "n8n", "git");
...
const buildScriptError = (error: unknown): Error => {
    ...
    const parts = [stderr, stdout, err?.message].filter((part): part is string => Boolean(part));
...
export const runGitScript = (scriptName: string, args: string[]): string => {
    const scriptFile = path.join(gitScriptsDir, scriptName);
    try {
