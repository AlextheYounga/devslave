...
# Concatenate all ticket files into one
cd "$codebase_path/$AGENT_FOLDER_NAME" || exit 1
zip_path="/tmp/agent_cache/tickets_${codebase_id}.zip"

if zip -r "$zip_path" ./tickets > /dev/null; then
    echo "$zip_path"
    exit 0
else
