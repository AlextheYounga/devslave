...
describe('POST /api/codebase/setup (SetupCodebaseController)', () => {
	...
	it('executes the setup script and returns 200 with IDs and stdout', async () => {
		...
		const res = await request(app)
			...
			.send({
				name: 'test-project',
				projectPath: tempDir,
				params: { scriptPath: testScriptPath },
			})
			.expect(200);
	...
	it("doesn't duplicate branch when master already exists", async () => {
		const codebase = await prisma.codebase.create({ data: { name: 'test-project', path: tempDir } });
		...
		const branches = await prisma.branch.findMany({ where: { codebaseId: codebase.id, name: 'master' } });
	...
	it('initializes git with master as the main branch and author info', async () => {
		...
		const headContent = fs.readFileSync(path.join(tempDir, '.git', 'HEAD'), 'utf-8').trim();
	...
	it('returns 500 and does not create records when setup script fails', async () => {
		...
		const res = await request(app)
			.post('/api/codebase/setup')
			.send({ name: 'test-project', projectPath: tempDir, params: { scriptPath: failingScriptPath } })
			.expect(500);
