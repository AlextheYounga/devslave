...
const GetAgentStatusHandlerMock =
  GetAgentStatusHandler as jest.MockedClass<typeof GetAgentStatusHandler>;
const StartAgentAndWaitHandlerMock =
  StartAgentAndWaitHandler as jest.MockedClass<typeof StartAgentAndWaitHandler>;
const StartAgentAndNotifyHandlerMock =
  StartAgentAndNotifyHandler as jest.MockedClass<typeof StartAgentAndNotifyHandler>;
...
describe("AgentController", () => {
  ...
  it("delegates to StartAgentHandler and returns success payload", async () => {
    ...
    expect(res.json.mock.calls[0][0].data).toMatchObject({ agentId: "agent-1" });
  });

  it("returns watch results", async () => {
    watchAgentHandle.mockResolvedValue({ agentId: "agent-22", status: "COMPLETED" });
  ...
  it("returns current status", async () => {
    getStatusHandle.mockResolvedValue({ agentId: "agent-7", status: "RUNNING" });
  ...
  it("merges start and watch results when executing synchronously", async () => {
    startAndWaitHandle.mockResolvedValue({ agentId: "agent-9", status: "COMPLETED" });
    const req: any = {
      body: {
        executionId: "exec-9",
        codebaseId: "code-9",
